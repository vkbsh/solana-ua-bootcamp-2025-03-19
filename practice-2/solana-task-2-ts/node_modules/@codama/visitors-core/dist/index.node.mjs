...
function getResolvedInstructionInputsVisitor(options = {}) {
  ...
  function resolveInstructionInput(instruction, input) {
    if (isNode3(input, "instructionAccountNode") && visitedAccounts.has(input.name) || isNode3(input, "instructionArgumentNode") && visitedArgs.has(input.name)) {
      return;
    }
    const isCircular = stack.some(({ kind, name }) => kind === input.kind && name === input.name);
    if (isCircular) {
      const cycle = [...stack, input];
      throw new CodamaError4(
        CODAMA_ERROR__VISITORS__CYCLIC_DEPENDENCY_DETECTED_WHEN_RESOLVING_INSTRUCTION_DEFAULT_VALUES,
        {
          cycle,
  ...
  function resolveInstructionAccount(instruction, account) {
    ...
    const localResolved = {
      ...
      isPda: getAllInstructionArguments(instruction).some(
        (argument) => isNode3(argument.defaultValue, "accountBumpValueNode") && argument.defaultValue.name === account.name
      ),
    ...
    switch (localResolved.defaultValue?.kind) {
      ...
      case "pdaValueNode":
        ...
        seeds.forEach((seed) => {
          if (!isNode3(seed.value, "accountValueNode")) return;
          const dependency = visitedAccounts.get(seed.value.name);
          if (dependency.resolvedIsOptional) {
            throw new CodamaError4(CODAMA_ERROR__VISITORS__CANNOT_USE_OPTIONAL_ACCOUNT_AS_PDA_SEED_VALUE, {
              instruction,
  ...
  function resolveInstructionDependencies(instruction, parent, dependencies) {
    dependencies.forEach((dependency) => {
      let input = null;
      if (isNode3(dependency, "accountValueNode")) {
        const dependencyAccount = instruction.accounts.find((a) => a.name === dependency.name);
        if (!dependencyAccount) {
          throw new CodamaError4(CODAMA_ERROR__VISITORS__INVALID_INSTRUCTION_DEFAULT_VALUE_DEPENDENCY, {
            dependency,
      ...
      } else if (isNode3(dependency, "argumentValueNode")) {
  ...
  return singleNodeVisitor("instructionNode", (node) => {
    ...
    const inputs = [
      ...
      ...node.arguments.filter((a) => {
        ...
        return a.defaultValue && !isNode3(a.defaultValue, VALUE_NODES2);
      }),
...
function getInstructionDependencies(input) {
  if (isNode3(input, "instructionNode")) {
  ...
  if (isNode3(input.defaultValue, ["accountValueNode", "accountBumpValueNode"])) {
    return [accountValueNode(input.defaultValue.name)];
  }
  if (isNode3(input.defaultValue, ["argumentValueNode"])) {
    return [argumentValueNode(input.defaultValue.name)];
  }
  if (isNode3(input.defaultValue, "pdaValueNode")) {
    const dependencies = /* @__PURE__ */ new Map();
    input.defaultValue.seeds.forEach((seed) => {
      if (isNode3(seed.value, ["accountValueNode", "argumentValueNode"])) {
        dependencies.set(seed.value.name, { ...seed.value });
      }
    });
    return [...dependencies.values()];
  }
  if (isNode3(input.defaultValue, "resolverValueNode")) {
    return input.defaultValue.dependsOn ?? [];
  }
  if (isNode3(input.defaultValue, "conditionalValueNode")) {
    return deduplicateInstructionDependencies([
...
import { CODAMA_ERROR__LINKED_NODE_NOT_FOUND, CodamaError as CodamaError5 } from "@codama/errors";
import {
  isNode as isNode4
} from "@codama/nodes";
var LINKABLE_NODES = [
...
var LinkableDictionary = class {
  ...
  getPathOrThrow(linkPath) {
    ...
    if (!linkablePath) {
      ...
      throw new CodamaError5(CODAMA_ERROR__LINKED_NODE_NOT_FOUND, {
        kind: linkNode.kind,
        linkNode,
        name: linkNode.name,
        path: linkPath
  ...
  getOrCreateProgramDictionary(linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const programNode2 = isNode4(linkableNode, "programNode") ? linkableNode : findProgramNodeFromPath(linkablePath);
  ...
  getOrCreateInstructionDictionary(programDictionary, linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const instructionNode2 = isNode4(linkableNode, "instructionNode") ? linkableNode : findInstructionNodeFromPath(linkablePath);
  ...
  getProgramDictionary(linkPath) {
    ...
    if (isNode4(linkNode, "programLinkNode")) {
  ...
  getInstructionDictionary(programDictionary, linkPath) {
    ...
    if (isNode4(linkNode, "instructionLinkNode")) {
...
import { isNode as isNode5 } from "@codama/nodes";
...
function getRecordLinkablesVisitor(linkables) {
  ...
  return pipe(
    voidVisitor(),
    (v) => interceptVisitor(v, (node, next) => {
      if (isNode5(node, LINKABLE_NODES)) {
